...
def parse_args():
    ...
    parser.add_argument("--indexer-ckpts", type=str, required=True)
...
def sparse_attention_forward_vectorized(attn_module, hidden_states, freqs_cis, x_real_len, cap_real_len, indexer, topk):
    ...
    with torch.amp.autocast(hidden_states.device.type, enabled=False):
        ...
        image_q = qf[:x_real_len]            # [Q, H, D]
        image_k = kf[:x_real_len]            # [K, H, D]
        image_v = vf[:x_real_len]            # [K, H, D]
        context_k = kf[x_real_len:]          # [C, H, D]
        context_v = vf[x_real_len:]          # [C, H, D]
        ...
        topk_idx = student_scores.topk(min(topk, x_real_len), dim=-1).indices  # [Q, Ksel]

        # Gather selected image keys/values for every query.
        gather_idx = topk_idx.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, image_k.shape[1], image_k.shape[2])
        selected_k = image_k.unsqueeze(0).expand(x_real_len, -1, -1, -1).gather(1, gather_idx)  # [Q, Ksel, H, D]
        selected_v = image_v.unsqueeze(0).expand(x_real_len, -1, -1, -1).gather(1, gather_idx)  # [Q, Ksel, H, D]
        ...
        sparse_image_out = torch.einsum("qhc,qchd->qhd", candidate_probs, candidate_v)  # [Q, H, D]

        # Non-image queries still use dense attention.
...
def make_sparse_model_fn(indexer_map, layer_ids, topk, stats_holder):
    ...
    def sparse_model_fn(
        ...
        **kwargs,
        ...
        if dit.siglip_embedder is not None:
            raise NotImplementedError("Stage5 script currently supports Z-Image-Turbo style path only.")
        ...
        for idx, layer in enumerate(dit.layers):
            if idx in layer_ids_set:
                ...
                sparse_attn_out, recall = sparse_attention_forward_vectorized(
                    attn_module=layer.attention,
                    hidden_states=hidden_input,
                    freqs_cis=unified_freqs_cis,
                    x_real_len=x_real_len,
                    cap_real_len=cap_real_len,
                    indexer=indexer_map[idx],
                    topk=topk,
                )
                per_layer_recalls[str(idx)] = recall
        ...
        x = -x
        stats_holder.append(per_layer_recalls)
        return x
...
def mean_recall(stats, layer_ids):
    values = []
    for item in stats:
...
def main():
    ...
    ckpt_paths = [x for x in args.indexer_ckpts.split(",") if x.strip()]
    if len(layer_ids) != len(ckpt_paths):
        raise ValueError("layer-ids and indexer-ckpts must have the same length")

    prompts = DEFAULT_PROMPTS[: args.num_prompts]
    pipe = build_pipe(args)
    indexer_map = {layer_id: load_indexer(ckpt_path, args.device) for layer_id, ckpt_path in zip(layer_ids, ckpt_paths)}
    ...
    for idx, prompt in enumerate(prompts):
        ...
        sparse_stats = []
        pipe.model_fn = make_sparse_model_fn(indexer_map, layer_ids, args.topk, sparse_stats)
        ...
        save_comparison_grid(
            ...
            sparse_image,
            "dense",
            f"vectorized sparse layers={','.join(map(str, layer_ids))} topk={args.topk}",
            grid_path,
        )
